...
namespace DocImageServer
{
    ...
    public class DocImageServer : IDocImageServer
    {
        ...
        public Dictionary<string, bool> scrapeTest(string hstNumber, string legalName)
        {
            ...
            bool error = false;
            int throwaway;
            bool isNumeric = int.TryParse(hstNumber, out throwaway);
            //Validate input here
            if (hstNumber == "")
            {
            ...
            else
            {
                ...
                {
                    returnDictionary.Add("HstNot9DigitsError", true);
                }
            ...
            if (resultNode.Count() != 10)
            {
                ...
                if (correctNode.InnerText.Contains("GST/HST number registered on this transaction date."))
                {
                    returnDictionary.Add("HstNumberIsValid", true);
                }
                else
                {
                    returnDictionary.Add("LegalNameIsImproperError", true);
                }
            }
            else
            {
